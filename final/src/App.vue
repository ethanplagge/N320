<template>
  ...
  <!--I attempted to use the RouterLink as a way to transfer data, but my research suggests that this is not possible-->
  <RouterView @parentSelectedSessions="addUserSessions"/>
</template>
...
export default {
  ...
  //the provide needs to be a function because it needs to send over an object containing multiple things, with the sessions data and the selected data
  provide() {
    return {
      //for some reason, the sessions array can be sent to children components easily, but the selectedSessions cannot be
      ...
      sessions: [
        {
          ...
          desc: 'As web developers, we have been building web applications the sme way for 25 years. We send a request, and we receive a response. We might get a whole page back, or we might get some data to render, but the common thread for almost every web application that has ever been built is the request/response cycle. What if we did not have to do it this way? What kind possibilities would open up to us?',
  ...
  methods: {
    addUserSessions(selectedSession){
      //adds the sessions, this should update everytime one is added, so it is okay with just an = sign
      this.selectedSessions.push(selectedSession);
      console.log("Session added in App.vue:", selectedSession);
      console.log("Updated selectedSessions in App.vue:", this.selectedSessions);
    }
  }
